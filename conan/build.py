...
def add_extra_appveyor_builds(builder):
    if os.getenv("CONAN_ARCHS") == "x86_64" and os.getenv("CONAN_VISUAL_VERSIONS") == "10":
        settings = {'compiler.version': '10', 'arch': 'x86_64', 'build_type': os.getenv("CONAN_BUILD_TYPES"),
                    'compiler.runtime': 'MT%s' % ('d' if os.getenv("CONAN_BUILD_TYPES") == "Debug" else '') ,
                    'compiler': 'Visual Studio'}
        options= {'flatbuffers:shared': False}
        builder.add(settings=settings, options=options, env_vars={}, build_requires={})
        settings = {'compiler.version': '10', 'arch': 'x86_64', 'build_type': os.getenv("CONAN_BUILD_TYPES"),
                    'compiler.runtime': 'MD%s' % ('d' if os.getenv("CONAN_BUILD_TYPES") == "Debug" else '') ,
                    'compiler': 'Visual Studio'}
        options= {'flatbuffers:shared': False}
        builder.add(settings=settings, options=options, env_vars={}, build_requires={})
        settings = {'compiler.version': '10', 'arch': 'x86_64', 'build_type': os.getenv("CONAN_BUILD_TYPES"),
